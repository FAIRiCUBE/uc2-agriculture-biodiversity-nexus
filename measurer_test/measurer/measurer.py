...
import psutil
from measurer.measurer_utilities import bytes_to
import tracemalloc
import platform
# import torch
import pandas as pd
...
#        self.cpu_gpu_description = description
    ...
    def write_out(self, csv_file):
        ...
        csv.loc[len(csv)] = {'Measure': 'Data size (MB)',
                             'Value': round(bytes_to(self.data_size, 'm'),2)}
        csv.loc[len(csv)] = {'Measure': 'Data read (MB)',
                             'Value': round(bytes_to(self.data_read, 'm'),2)}
        csv.loc[len(csv)] = {'Measure': 'Data written (MB)',
                             'Value': round(bytes_to(self.data_written, 'm'),2)}
        csv.loc[len(csv)] = {'Measure': 'Main memory available (MB)',
                             'Value': round(bytes_to(self.main_memory_available, 'm'),2)}
        csv.loc[len(csv)] = {'Measure': 'Main memory consumed (MB)',
                             'Value': round(bytes_to(self.main_memory_consumed, 'm'), 2)}
        csv.loc[len(csv)] = {'Measure': 'Sum of allocated variable sizes (MB)',
                             'Value': round(bytes_to(self.sum_of_allocated_variable_sizes, 'm'),2)}
        ...
        csv.loc[len(csv)] = {'Measure': 'Network traffic (MB)',
                             'Value': round(bytes_to(self.network_traffic, 'm'),2)}
